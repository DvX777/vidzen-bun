...
export default function WavvyPlayerWrapper({type,id,season,episode}){
  ...
  const mediaMetaRef=useRef({title:'',poster_path:'',backdrop_path:''});
  // Seek guard — prevents seek-induced HLS errors from triggering autoFallback
  ...
  // double-tap seek state for mobile
  const lastTapRef=useRef({side:null,ts:0});
  const [tapFlash,setTapFlash]=useState(null); // 'left'|'right'|null

  // Fetch TMDB metadata (poster + title for progress saves)
  useEffect(()=>{
    fetch(`https://api.themoviedb.org/3/${type}/${id}?api_key=${TMDB_KEY}`)
      .then(r=>r.json()).then(d=>{
        if(d.backdrop_path) setPoster(`https://image.tmdb.org/t/p/w1280${d.backdrop_path}`);
  ...
  // VidLink-compatible postMessage listener (write to vidLinkProgress for external embeds)
  useEffect(()=>{
    const handler=(event)=>{
      if(event.data?.type==='MEDIA_DATA'){
        try{localStorage.setItem('vidLinkProgress',JSON.stringify(event.data.data));}catch{}
      }
    };
  ...
  const playSource=useCallback((url,type_,prov,srcObj)=>{
    ...
    const restorePosition=()=>{
      ...
      const prog=getProgress(mt,mi,ms||undefined,me||undefined);
      if(prog?.watched>5)v.currentTime=prog.watched;
    };
    if(type_==="hls"&&Hls.isSupported()){
      ...
      h.on(Hls.Events.MANIFEST_PARSED,(_,d)=>{
        ...
        netErrCount=0;
        setHlsLevels(d.levels);onReady();
        const play=()=>{restorePosition();v.play().catch(()=>{});};
        if(v.readyState>=1)play();else v.addEventListener('loadedmetadata',play,{once:true});
      });
      h.on(Hls.Events.FRAG_BUFFERED,()=>{if(!job.cancelled){netErrCount=0;setBuffering(false);}});
      h.on(Hls.Events.ERROR,(_,d)=>{
        if(job.cancelled||!d.fatal)return;
        const recentSeek=Date.now()-lastSeekRef.current<5000;
        if(d.type===Hls.ErrorTypes.MEDIA_ERROR&&d.details!=='bufferAddCodecError'){
          // Media decode errors: attempt in-place recovery first
          h.recoverMediaError();return;
        }
        if(d.type===Hls.ErrorTypes.NETWORK_ERROR){
          // Transient network error (common after seeks into unbuffered positions)
          // Retry fragment load instead of nuking the session
          if(recentSeek||netErrCount<3){
            netErrCount++;
            try{h.startLoad();}catch{}
            return;
          }
        }
        // Genuine unrecoverable fatal — tear down and try next provider
        netErrCount=0;
  ...
  const autoFallback=useCallback(async()=>{
    ...
    for(const prov of ALL_PROVIDERS){
      ...
      serverNum++;
      try{
        setSwitchMsg(`Trying server ${serverNum}…`);setLoading(true);
    ...
    setErr('Video failed to load - Please refresh the page');
  ...
  useEffect(()=>{
    ...
    const go=async()=>{
      ...
      if(!cancelled){setErr('All Servers Failed - Please refresh the page');setLoading(false);}
    };
    go();
  ...
  // Remove old redundant HLS-only progress restore effect (now handled inside playSource)
  ...
  // Progress save — watchProgress.js (VidLink-compatible, includes title+poster broadcast)
  useEffect(()=>{
    progRef.current=setInterval(()=>{
      if(videoRef.current&&isPlaying&&cur>5){
        const meta=mediaMetaRef.current;
        saveProgress({
          type,id:Number(id),season:season?Number(season):undefined,
          episode:episode?Number(episode):undefined,
          watched:cur,duration:dur,
          title:meta.title,poster_path:meta.poster_path,backdrop_path:meta.backdrop_path,
        });
      }
    },5000);
  ...
  return (
    <div suppressHydrationWarning ref={containerRef} style={{position:"fixed",inset:0,background:"#000",zIndex:9999,userSelect:"none",cursor:showCtrl?"default":"none"}}
      ...
      {isMobile&&!loading&&!err&&activeSource?.url&&(
        <>
          <div style={{position:"absolute",top:0,left:0,width:"40%",bottom:0,zIndex:38,WebkitTapHighlightColor:"transparent"}}
            onDoubleClick={e=>{e.stopPropagation();skipB();setTapFlash('left');setTimeout(()=>setTapFlash(null),600);}}/>
          <div style={{position:"absolute",top:0,right:0,width:"40%",bottom:0,zIndex:38,WebkitTapHighlightColor:"transparent"}}
            onDoubleClick={e=>{e.stopPropagation();skipF();setTapFlash('right');setTimeout(()=>setTapFlash(null),600);}}/>
          {tapFlash&&<div style={{position:"absolute",top:"50%",transform:"translateY(-50%)",left:tapFlash==='left'?"8%":undefined,right:tapFlash==='right'?"8%":undefined,background:"rgba(255,255,255,0.15)",borderRadius:"50%",width:80,height:80,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none",zIndex:39,backdropFilter:"blur(4px)",transition:"opacity 0.3s"}}>
            <svg width="32" height="32" viewBox="0 0 24 24" fill="white"><path d={tapFlash==='left'?"M11 5L5 12l6 7M5 12h14":"M13 5l6 7-6 7M19 12H5"}/></svg>
          </div>}
        </>
      )}
      ...
      {!loading&&!buffering&&!err&&activeSource?.url&&(
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",gap:isMobile?48:80,zIndex:40,opacity:showCtrl?1:0,transition:"opacity 0.2s",pointerEvents:"none"}}>
          <button onClick={e=>{e.stopPropagation();skipB();}} style={{background:"none",border:"none",color:"#fff",cursor:"pointer",padding:0,display:"flex",transition:"transform 0.15s",pointerEvents:"auto"}} aria-label="Back 10s">
            <svg xmlns="http://www.w3.org/2000/svg" width={isMobile?44:60} height={isMobile?44:60} viewBox="0 0 24 24"><g fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.5"><path strokeLinejoin="round" d="m12 5l-1.104-1.545c-.41-.576-.617-.864-.487-1.13c.13-.268.46-.283 1.12-.314Q11.763 2 12 2c5.523 0 10 4.477 10 10s-4.477 10-10 10S2 17.523 2 12a9.99 9.99 0 0 1 4-8"/><path d="M7.992 11.004C8.52 10.584 9 9.891 9.3 10.02s.204.552.204 1.212v4.776m6.498-3.408c0-1.38.066-1.752-.198-2.196s-.924-.406-1.584-.406s-1.14-.038-1.458.322c-.39.42-.222 1.2-.27 2.28c.108 1.44-.186 2.58.264 3.06c.324.396.9.336 1.584.348c.68-.008 1.092.024 1.428-.36c.372-.336.192-1.668.234-3.048Z"/></g></svg>
          </button>
          <button onClick={e=>{e.stopPropagation();togglePlay();}} style={{background:"none",border:"none",color:"#fff",cursor:"pointer",padding:0,display:"flex",transition:"transform 0.15s",pointerEvents:"auto"}} aria-label={isPlaying?"Pause":"Play"}>
            {isPlaying
              ?<svg xmlns="http://www.w3.org/2000/svg" width={isMobile?64:90} height={isMobile?64:90} viewBox="0 0 24 24"><path fill="currentColor" d="M2 6c0-1.886 0-2.828.586-3.414S4.114 2 6 2s2.828 0 3.414.586S10 4.114 10 6v12c0 1.886 0 2.828-.586 3.414S7.886 22 6 22s-2.828 0-3.414-.586S2 19.886 2 18zm12 0c0-1.886 0-2.828.586-3.414S16.114 2 18 2s2.828 0 3.414.586S22 4.114 22 6v12c0 1.886 0 2.828-.586 3.414S19.886 22 18 22s-2.828 0-3.414-.586S14 19.886 14 18z"/></svg>
              :<svg xmlns="http://www.w3.org/2000/svg" width={isMobile?64:90} height={isMobile?64:90} viewBox="0 0 24 24"><path fill="currentColor" d="M21.409 9.353a2.998 2.998 0 0 1 0 5.294L8.597 21.614C6.534 22.737 4 21.277 4 18.968V5.033c0-2.31 2.534-3.769 4.597-2.648z"/></svg>}
          </button>
          <button onClick={e=>{e.stopPropagation();skipF();}} style={{background:"none",border:"none",color:"#fff",cursor:"pointer",padding:0,display:"flex",transition:"transform 0.15s",pointerEvents:"auto"}} aria-label="Forward 10s">
      ...
      {activeCue&&<div style={{position:"absolute",bottom:isMobile?90:100,left:0,right:0,textAlign:"center",pointerEvents:"none",zIndex:35}}>
        <span style={{background:"rgba(0,0,0,0.82)",color:"#fff",padding:isMobile?"5px 16px":"4px 14px",borderRadius:4,fontSize:isMobile?17:16,fontFamily:"system-ui,sans-serif",maxWidth:"90%",display:"inline-block"}}>{activeCue.text}</span>
      </div>}
      ...
      <div style={{position:"absolute",top:8,left:8,zIndex:30}} onClick={e=>e.stopPropagation()}>
        ...
        {showMenu&&<div style={{position:"absolute",top:54,left:0,background:"rgba(18,18,22,0.95)",backdropFilter:"blur(24px)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:20,boxShadow:"0 32px 64px rgba(0,0,0,0.7)",width:isMobile?`min(280px,calc(100vw - 24px))`:280,maxHeight:isMobile?"50vh":"70vh",overflowY:"auto",zIndex:50,fontFamily:"system-ui,sans-serif"}}>
            ...
            {SERVERS.map(srv=>{
              ...
              return(
                <button key={srv.id}
                  ...
                  <img src={`https://flagcdn.com/20x15/${srv.cc}.png`} width="20" height="15" alt={srv.cc} style={{borderRadius:2,flexShrink:0,objectFit:"cover"}}/>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontWeight:500,fontSize:14,lineHeight:1.2}}>{srv.name}</div>
                    <div style={{fontSize:11,marginTop:3,color:available?"rgba(255,255,255,0.38)":"rgba(255,255,255,0.22)",fontStyle:available?"normal":"italic"}}>{available?srv.dubLabel:"Unavailable"}</div>
                  </div>
                  {isActive
                    ?<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
                    :available
                      ?<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/></svg>
                      :<span style={{width:14}}/>}
                </button>);
            })}
          </div>
      ...
      <div style={{position:"absolute",bottom:0,left:0,right:0,opacity:showCtrl?1:0,transition:"opacity 0.3s",pointerEvents:showCtrl?"auto":"none"}} onClick={e=>e.stopPropagation()}>
        <div style={{background:"linear-gradient(to top,rgba(0,0,0,0.92),rgba(0,0,0,0.6) 60%,transparent)",padding:isMobile?"28px 12px 10px":"32px 16px 16px"}}>

          {/* Progress bar — taller hit area on mobile */}
          <div style={{position:"relative",marginBottom:isMobile?12:16,height:isMobile?32:24,display:"flex",alignItems:"center"}}>
            <div style={{position:"absolute",left:0,right:0,top:"50%",transform:"translateY(-50%)",height:isMobile?8:6,background:"rgba(255,255,255,0.2)",borderRadius:4}}/>
            <div style={{position:"absolute",left:0,top:"50%",transform:"translateY(-50%)",height:isMobile?8:6,width:`${bpct}%`,background:"rgba(255,255,255,0.3)",borderRadius:4}}/>
            <div style={{position:"absolute",left:0,top:"50%",transform:"translateY(-50%)",height:isMobile?8:6,width:`${pct}%`,background:"linear-gradient(90deg,#3b82f6,#60a5fa)",borderRadius:4}}/>
            ...
            <div style={{position:"absolute",top:"50%",transform:"translateY(-50%)",width:isMobile?16:12,height:isMobile?16:12,background:"#fff",borderRadius:"50%",boxShadow:"0 0 6px rgba(0,0,0,0.6)",pointerEvents:"none",left:`calc(${pct}% - ${isMobile?8:6}px)`}}/>
          </div>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:isMobile?10:16}}>
              ...
              {/* Volume slider — desktop only; mobile uses mute tap */}
              {!isMobile&&<div style={{position:"relative",width:80,height:6,background:"rgba(255,255,255,0.2)",borderRadius:3}}>
              ...
              <span style={{color:"#fff",fontSize:isMobile?13:12,fontFamily:"system-ui,sans-serif",whiteSpace:"nowrap",fontVariantNumeric:"tabular-nums"}}>
                {fmt(cur)} / {fmt(dur)}
              </span>
            </div>

            {/* Right: CC (standalone) + PiP + Settings + Fullscreen */}
            <div style={{display:"flex",alignItems:"center",gap:isMobile?10:16}}>
              {/* CC button — always standalone, outside settings */}
              {subs.length>0&&<button
                onClick={()=>{if(activeSub){setActiveSub(null);}else{setShowSettings(v=>!v);setSettSect('captions');}}}
                style={{background:"none",border:"none",color:activeSub?"#3b82f6":"rgba(255,255,255,0.85)",cursor:"pointer",padding:0,display:"flex"}}
                onMouseEnter={e=>e.currentTarget.style.color="#fff"}
                onMouseLeave={e=>e.currentTarget.style.color=activeSub?"#3b82f6":"rgba(255,255,255,0.85)"}
                aria-label="Captions">
              ...
              {!isMobile&&<button onClick={togPiP} style={{background:"none",border:"none",color:"rgba(255,255,255,0.85)",cursor:"pointer",padding:0,display:"flex"}} onMouseEnter={e=>e.currentTarget.style.color="#fff"} onMouseLeave={e=>e.currentTarget.style.color="rgba(255,255,255,0.85)"} aria-label="PiP">
              ...
              <div style={{position:"relative"}}>
                ...
                {showSettings&&<div style={{position:"absolute",bottom:isMobile?62:52,right:0,background:"rgba(0,0,0,0.9)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:16,boxShadow:"0 25px 50px rgba(0,0,0,0.6)",width:isMobile?`min(220px,calc(100vw - 32px))`:220,zIndex:50,fontFamily:"system-ui,sans-serif",overflow:"hidden"}}>
